...
**TaskIQ + Redis** отделяет HTTP-запрос от работы с внешней системой. API быстро отвечает
клиенту, а подтверждение выполняется в worker. TaskIQ используется как разрешенная асинхронная
альтернатива Celery: worker остается async-native и использует тот же паттерн `AsyncSession`, что и
